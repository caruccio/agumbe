...
def replicateSecrets(secret_name, target_namespace):
    ...
    for namespace in target_namespace[0]:
        ...
        url = '{0}/api/v1/namespaces/{1}/secrets'.format(base_url, namespace, secret_name)

        post_response = requests.post(url, data=json.dumps(data), headers={'Content-Type': 'application/json'})
        print("xxx", url, data)
